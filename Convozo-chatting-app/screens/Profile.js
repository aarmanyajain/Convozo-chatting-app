...
const Profile = () => {
  ...
  // Check if auth.currentUser exists before accessing its properties
  const displayNameInitial = auth?.currentUser?.displayName
    ? auth.currentUser.displayName
        .split(" ")
        .reduce((prev, current) => `${prev}${current[0]}`, "")
    : "";
  const emailInitial = auth?.currentUser?.email
